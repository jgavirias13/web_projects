<?php
    if($_POST){
        ...
        if($usuario and $password){
            ...
            $query = "Select usuario from usuarios where usuario = '".mysqli_real_escape_string($conexion, $usuario)."'";
            $resultado = mysqli_query($conexion, $query);
            if(mysqli_num_rows($resultado) == 0){
                $query = "Insert into usuarios (usuario, password) values('".mysqli_real_escape_string($conexion, $usuario)."', '".mysqli_real_escape_string($password)."')";
                $resultado = mysqli_query($conexion, $query);
                if(!$resultado){
